"use client";
import { motion } from "framer-motion";
import { sekolah } from "../data/spmb";

export default function Footer() {
  return (
    <footer className="footer">
      <div className="footer-warning-wrap">
        <motion.div
          className="warning-banner footer-warning-banner"
          initial={{ opacity: 0, scale: 0.98 }}
          whileInView={{ opacity: 1, scale: 1 }}
          viewport={{ once: true }}
          transition={{ duration: 0.45 }}
        >
          <div className="warning-shimmer" />

          <div className="warning-content">
            <motion.div
              className="warning-icon-wrapper"
              animate={{ scale: [1, 1.05, 1], rotate: [0, -2, 2, -2, 0] }}
              transition={{ repeat: Infinity, repeatDelay: 4, duration: 0.8 }}
              style={{ display: "flex", alignItems: "center" }}
            >
              <img
                src="/slogan.png"
                alt="Slogan No Titip No Jastip"
                style={{
                  height: "58px",
                  objectFit: "contain",
                  filter: "drop-shadow(0 4px 6px rgba(0,0,0,0.2))",
                }}
              />
            </motion.div>

            <div className="warning-text">
              <span className="warning-label">PERINGATAN RESMI</span>
              <p className="warning-message">
                Proses SPMB{" "}
                <strong className="warning-highlight">100% GRATIS!</strong>{" "}
                <strong className="warning-highlight">BEBAS PUNGUTAN!</strong>
              </p>
              <p className="warning-sub">
                Laporkan jika ada pungutan kepada pihak berwenang
              </p>
            </div>

            <motion.div
              className="warning-badge"
              animate={{ scale: [1, 1.05, 1] }}
              transition={{ repeat: Infinity, duration: 2 }}
            >
              GRATIS
            </motion.div>
          </div>
        </motion.div>
      </div>

      <div className="footer-content">
        {/* Kiri: Identitas Sekolah */}
        <motion.div
          className="footer-col footer-col-brand"
          initial={{ opacity: 0, y: 12 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
        >
          <div className="footer-brand-row">
            <img src="/logosmkn4.png" alt="Logo SMKN 4" className="footer-logo-img" />
            <div className="footer-brand-text">
              <p className="footer-school">{sekolah.nama}</p>
              <p className="footer-slogan">{sekolah.slogan}</p>
            </div>
          </div>
          <p className="footer-copy">© {sekolah.tahun} · Informasi SPMB Resmi</p>
        </motion.div>

        {/* Tengah: Kontak Person + Sosmed */}
        <motion.div
          className="footer-col footer-col-center"
          initial={{ opacity: 0, y: 12 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ delay: 0.05 }}
        >
          <span className="footer-col-label">Kontak SPMB 2026</span>
          <div className="footer-contact-row">
            <a
              href="https://wa.me/6285747871815"
              target="_blank"
              rel="noreferrer"
              className="footer-contact-chip"
            >
              <span className="chip-icon">💬</span>
              <span className="chip-label">CP 1</span>
              <span className="chip-value">{sekolah.cp1}</span>
            </a>
            <a
              href="https://wa.me/6281914637994"
              target="_blank"
              rel="noreferrer"
              className="footer-contact-chip"
            >
              <span className="chip-icon">💬</span>
              <span className="chip-label">CP 2</span>
              <span className="chip-value">{sekolah.cp2}</span>
            </a>
          </div>
          <div className="footer-socmed-row">
            <a
              href="https://www.instagram.com/smkn4surakarta/"
              target="_blank"
              rel="noreferrer"
              className="socmed-pill instagram"
              title="Instagram SMKN 4 Surakarta"
            >
              <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.051.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/>
              </svg>
            </a>
            <a
              href="https://www.facebook.com/smkn4surakarta"
              target="_blank"
              rel="noreferrer"
              className="socmed-pill facebook"
              title="Facebook SMKN 4 Surakarta"
            >
              <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor">
                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
              </svg>
            </a>
            <a
              href="https://www.youtube.com/@SMKNegeri4Surakarta"
              target="_blank"
              rel="noreferrer"
              className="socmed-pill youtube"
              title="YouTube SMKN 4 Surakarta"
            >
              <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor">
                <path d="M23.498 6.163a3.003 3.003 0 00-2.11-2.11C19.518 3.545 12 3.545 12 3.545s-7.518 0-9.388.507a3.003 3.003 0 00-2.11 2.11C0 8.033 0 12 0 12s0 3.967.502 5.837a3.003 3.003 0 002.11 2.11c1.87.507 9.388.507 9.388.507s7.518 0 9.388-.507a3.003 3.003 0 002.11-2.11C24 15.967 24 12 24 12s0-3.967-.502-5.837zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
              </svg>
            </a>
            <strong className="socmed-handle">{sekolah.sosmed}</strong>
          </div>
        </motion.div>

        {/* Kanan: Link Portal Resmi */}
        <motion.div
          className="footer-col footer-col-links"
          initial={{ opacity: 0, y: 12 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ delay: 0.1 }}
        >
          <span className="footer-col-label">Portal & Tautan Resmi</span>
          <div className="footer-links-stack">
            <a
              href={`https://${sekolah.website}`}
              target="_blank"
              rel="noreferrer"
              className="footer-link-card school"
            >
              <span className="link-icon">🏫</span>
              <div className="link-text">
                <span className="link-title">Website Sekolah</span>
                <span className="link-url">{sekolah.website}</span>
              </div>
              <span className="link-arrow">→</span>
            </a>
          </div>
        </motion.div>
      </div>
    </footer>
  );
}
